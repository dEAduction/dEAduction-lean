...
example (H : A ⊆ B ∧ B ⊆ A) : 0=0 :=
begin
    have P := theorem.double_inclusion H,
    try { 
        have Q := exercise.inter_distributive_union H 
        <|> have Q := @exercise.inter_distributive_union
    },
...
-----------------------------------------
-----------------------------------------
namespace complementaire -- sous-section 2
-----------------------------------------
-----------------------------------------
variables  {A B : set X}
variables {I : Type} {E F : I → set X}
/-
unknown identifier 'goals_analysis'
-/
...
variables  (A A': set X) 
variables {Y: Type} {f: X → Y} {f': X → Y} (B B': set Y)
...
namespace injections_and_surjections

def injective (h: X → Y) := set.inj_on h (set.univ)
def surjective (h: X → Y) := set.inj_on h (set.univ)

lemma definition.injection : injective f ↔  ( ∀ x y : X, f(x) = f(y) → x = y ) :=
begin
    unfold injective, unfold set.inj_on,
    finish
end
...
end injections_and_surjections
