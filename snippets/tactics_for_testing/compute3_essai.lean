...
meta def compute_n (n: nat)  (cfg: compute_config := {}) : tactic string :=
    do  t_ineq ← target_lt_or_gt, t_abs ← target_ite, match (t_ineq, t_abs) with
        ...
        -- Essentially try successively specific tactics on the target and linarith
            let ineq_tacs_1 := 
                iterate_and_return_code n $ or_else_and_return_code $
...
meta def compute_and_trace_code (n: nat) (id: string)  (cfg: compute_config := {}):
tactic unit :=
do apply_and_trace_code id (tactic.interactive.compute_n n cfg)
