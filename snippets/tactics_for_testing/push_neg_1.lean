...
meta def normalize_negations_1 (t : expr) : tactic (expr × expr) :=
do (_, e, pr) ← simplify_top_down ()
                   (λ _, λ e, do
                       oepr ← transform_negation_step e,
                       match oepr with
...
meta def push_neg_1_at_hyp (h : name) : tactic unit :=
do H ← get_local h,
   t ← infer_type H,
   (e, pr) ← normalize_negations_1 t,
   replace_hyp H e pr,
   skip

meta def push_neg_1_at_goal : tactic unit :=
do H ← target,
   (e, pr) ← normalize_negations_1 H,
   replace_target e pr
end push_neg
...
-/
meta def tactic.interactive.push_neg_1 : parse location → tactic unit
| (loc.ns loc_l) := loc_l.mmap'
                      (λ l, match l with
                            | some h := do push_neg_1_at_hyp h,
                                            try `[simp only [push_neg.not_eq] at h { eta := ff }]
                            | none   := do push_neg_1_at_goal,
                                            try `[simp only [push_neg.not_eq] { eta := ff }]
                            end)
| loc.wildcard := do
    push_neg_1_at_goal,
    local_context >>= mmap' (λ h, push_neg_1_at_hyp (local_pp_name h)) ,
