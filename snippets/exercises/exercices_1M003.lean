...
notation `[` a `,`b `]` := Icc (a:ℝ) (b:ℝ) 
notation [parsing_only] `]` a `,`b `[` := Ioo (a:ℝ) (b:ℝ) 
...
lemma definition.intervalle_ouvert {a b x : ℝ} :
x ∈ Ioo a b ↔ (a < x and x < b)
:=
begin
    sorry
end
...
begin
    intro h,
    rw definition.intervalle_ferme at h,
    cases h with ha hb, exact hb,
end
...
lemma exercise.essai
(x:ℝ) (E F : set ℝ) (H1: x ∈ E) (H2: (∀ x, x ∈ F ↔ -x ∈ E)) :
est_majore E → est_minore (F)
:=
begin
    sorry
end
...
lemma exercise.axiome_borne_inf 
